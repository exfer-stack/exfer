...
async fn main() {
    ...
    match cli.command {
        Commands::Node {
            ...
            if let Err(e) = run_node(bind, peers, datadir, None, repair_perms, rpc_bind, verify_all, no_assume_valid, purge_bans, no_auto_migrate, rebuild_state, full_verify, build_spent_by_index, false).await {
        ...
        Commands::Mine {
            ...
            if let Err(e) =
                run_node(bind, peers, datadir, Some(pubkey), repair_perms, rpc_bind, verify_all, no_assume_valid, purge_bans, no_auto_migrate, rebuild_state, full_verify, build_spent_by_index, false).await
            {
                error!("Node failed to start: {e}");
                std::process::exit(1);
            }
...
async fn run_node(
    ...
    // node, so there is no IBD, no peers to dial, and no discovery. Mined
    // blocks are committed directly via `process_block`, so the sync
    // manager is not needed to advance the chain.
    if !devnet {
        ...
        drop(peer_events_rx);
        info!("devnet: isolated single-node chain — sync/outbound/discovery disabled");
    }
